...
func (con RoleController) DoEdit(c *gin.Context) {
	...
	if err2 != nil {
		con.Error(c, "修改数据失败", "/admin/role/edit?id="+models.IntToString(id))
	} else {
...
func (con RoleController) DoAuth(c *gin.Context) {
	...
	fmt.Println("/admin/role/auth?id=?" + models.IntToString(roleId))
	// c.String(200, "DoAuth")
	// admin/role/auth?id=9
	con.Success(c, "授权成功", "/admin/role/auth?id="+models.IntToString(roleId))
}
